...
很多人做"学生版"软件，思路是"简化版"：按钮少一点、选项少一点、功能砍一点。做完发现学生根本不用。简化不是把复杂藏起来，而是把该露的露出来。
...
- 点"修改消息"不是就在气泡里就地改，而是在页面最底下冒出一个编辑框，我得先找到它再改。
...
持久副本还带路径白名单：`uploads/` 目录 `canonicalize` 校验，防符号链接逃逸。一句话：学生只看见"我传了张图片"，路径是内核内部的事。
...
这不是"学生版才需要"的防护，是任何渲染模型输出的应用都该有的底线。
