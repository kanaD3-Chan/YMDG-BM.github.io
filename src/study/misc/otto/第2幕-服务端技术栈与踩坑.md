...
### 15. tokio-serial 在 USB 拔插时 panic（exit 101）

`tokio-serial` 底层 mio-serial，当 `/dev/ttyUSB0` 物理拔掉时，已打开的 fd 变为无效。异步 I/O 轮询该 fd 时不是返回错误，而是在 tokio reactor 内部 panic，导致整个进程 exit 101。systemd 虽然能 `Restart=always`，但断连恢复依赖 systemd 重拉进程，不够优雅。

**解决：** 切换到同步 `serialport` 包，在 `tokio::task::spawn_blocking` 中阻塞读取，通过 `tokio::sync::mpsc` channel 将行数据交回异步上下文。同步 `read()` 在 USB 断连时返回 `Err`，被 `match` 捕获后干净退出阻塞线程，外层循环自动重试 `find_port()` 完成重连。全程不 panic、不 crash、不依赖 systemd 重启。